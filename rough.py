...
print(f"-> start rough, rank: {C.local_rank}")
if C.local_rank == 0:
    # logger
...
if C.optimizer == 'SGD':
    optimizer = optim.SGD(params=model.parameters(),
                          ...
                          nesterov=C.nesterov)
elif C.optimizer in ['AdamW', 'RAdam']:
...
for epoch in range(C.nepochs):
    ...
    for x, y in vbar:
        ...
        with torch.no_grad():
            lgt = val_model(x)
            ls = F.cross_entropy(lgt, y.long())
        _val_acc = accuracy(lgt, y)[0]
        val_ls.update(ls.item())
        val_acc.update(_val_acc)
        vbar.set_description('epoch: {}, '
                             'val_ls: {:.4f}, '
                             'val_acc: {:.4f}, '.format(epoch,
                                                        val_ls.avg,
                                                        val_acc.avg))

    if C.ddp in ['slurm', 'normal']:
        dist.all_reduce(val_acc.avg / C.world_size, op=dist.ReduceOp.SUM)
    ...
    if C.local_rank == 0:
        ...
        if val_acc.avg > best_acc:
            ...
            prCyan(f"epoch: {epoch}, val_acc: {val_acc.avg}, best ckpt saved")
            best_acc = val_acc.avg
            best_epoch = epoch
            plateau_count = 0
        else:
            plateau_count += 1
            prGreen("epoch: {}, val_acc: {:.4f} | p{:02d}".format(epoch, val_acc.avg, plateau_count))

        C.summary_writer.add_scalar('val_ls', val_ls.avg, gs)
        C.summary_writer.add_scalar('val_acc', val_acc.avg, gs)

        row = {'epoch': str(epoch),
               'train_ls': '{:.4f}'.format(train_ls.avg),
               'train_acc': '{:.4f}'.format(train_acc.avg),
               'val_ls': '{:.4f}'.format(val_ls.avg),
               'val_acc': '{:.4f}'.format(val_acc.avg),
...
if C.local_rank == 0:
    ...
    prPurple(f'best acc: {best_acc}, epoch: {best_epoch}')
